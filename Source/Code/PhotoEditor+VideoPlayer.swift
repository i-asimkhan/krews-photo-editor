...
extension PhotoEditorViewController {
    ...
    func prepareVideoView(url: URL) {
        
        DispatchQueue.main.async {
            
            self.avPlayerViewController.view.frame = self.avPlayerView.bounds
            self.addChild(self.avPlayerViewController)
            self.avPlayerView.addSubview(self.avPlayerViewController.view)
            
            self.avPlayer = AVPlayer(url: url as URL)
            self.avPlayerViewController.player = self.avPlayer
            self.avPlayerViewController.videoGravity = AVLayerVideoGravity(rawValue: AVLayerVideoGravity.resizeAspectFill.rawValue)
            self.avPlayerViewController.showsPlaybackControls = false
            self.avPlayerViewController.setValue(false, forKey: "canHidePlaybackControls")
            ...
            if let player = self.avPlayer {
                ...
                NotificationCenter.default.addObserver(
                    self,
                    selector: #selector(self.videoCompleted),
            ...
            self.avPlayerViewController.view.isUserInteractionEnabled = false
        }
    }
    ...
    public override func observeValue(forKeyPath keyPath: String?, of object: Any?, change: [NSKeyValueChangeKey : Any]?, context: UnsafeMutableRawPointer?) {
        if keyPath == "rate" , let player = self.avPlayer {
            if player.rate > 0 {
                print("video started")
            }
        }
